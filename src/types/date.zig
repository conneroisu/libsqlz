...
test "Compare dates" {
    ...
    const date_now = try Date.now();
    const relative_date = try date.from(date_now);
...
test "Get weekday" {
    var date = Date.init(2002, 7, 23);
    const date_weekday = try date.weekday();
...
test "Check leap year" {
    ...
    const leap_year = try date.isLeapYear();
...
test "Date to string" {
    var date = Date.init(2002, 7, 23);
    const date_as_string = try date.toString(.@"YYYY-MM-DD");
